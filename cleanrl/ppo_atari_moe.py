...
def load_agent(env, model_dir, wrapper):
    ckpt = torch.load(f"{model_dir}/{wrapper}.cleanrl_model", map_location=device, weights_only=False)
...
class PPOAgent(nn.Module):
    def __init__(self, env, normalize=True):
        ...
        with torch.no_grad():
            f = self.network(torch.zeros(dims))
    ...
    def forward(self, x):
        x = torch.from_numpy(self.norm_img(x)).float().unsqueeze(0).to(device)
...
class MoEWrapper(ObservationWrapper):
    def __init__(self, env, model_dir):
        ...
        for wrapper in env.wrappers:  # noqa: should wrap an occam multi wrapper
            self.agents[wrapper] = load_agent(env.wrappers[wrapper], model_dir, wrapper)

        super().__init__(env)
        self.observation_space = gym.spaces.Box(0, 1, shape=[env.unwrapped.action_space.n * len(self.agents)])  # noqa: action space is discrete
...
class MoEAgent(nn.Module):
    def __init__(self, envs, device, layer_dims=(128, 64), weighted_sum=True):
        ...
        self.critic = layer_init(nn.Linear(in_dim, 1), std=1)
