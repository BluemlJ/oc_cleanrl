...
class PPOAgent(nn.Module):
    def __init__(self, envs):
        ...
        self.network = nn.Sequential(
            nn.Linear(dims[1], emb_dim),
            Encoder(emb_dim, 4, 1),
            nn.Flatten(),
        )
        self.actor = layer_init(nn.Linear(dims[0] * emb_dim, envs.action_space.n), std=0.01)
        self.critic = layer_init(nn.Linear(dims[0] * emb_dim, 1), std=1)
...
if __name__ == "__main__":
    ...
    # Create RTPT object
    rtpt = RTPT(name_initials='CD', experiment_name='TrainingAtari',
                max_iterations=args.num_iterations)

    # Start the RTPT tracking
    rtpt.start()

    min_level = args.logging_level
