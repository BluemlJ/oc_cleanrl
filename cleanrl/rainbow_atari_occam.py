...
PrioritizedBatch = collections.namedtuple(
    "PrioritizedBatch", ["observations", "actions", "rewards",
                         "next_observations", "dones", "indices", "weights"]
)
...
class SumSegmentTree:
    ...
    def _propagate(self, idx):
        parent = (idx - 1) // 2
        while parent >= 0:
            self.tree[parent] = self.tree[parent * 2 + 1] + \
...
class PrioritizedReplayBuffer:
    def __init__(self, capacity, obs_shape, device, n_step, gamma, alpha=0.6, beta=0.4, eps=1e-6):
        ...
        self.buffer_next_obs = np.zeros(
            (capacity,) + obs_shape, dtype=np.uint8)
    ...
    def sample(self, batch_size):
        ...
        probs = np.array([self.sum_tree.tree[idx + self.capacity - 1]
                         for idx in indices])
        weights = (self.size * probs / p_total) ** -self.beta
        weights = weights / weights.max()
        samples["weights"] = torch.from_numpy(
            weights).to(self.device).unsqueeze(1)
...
"""
    ...
    for global_step in pbar:
        ...
        rb.beta = min(
            ...
            (1.0 - args.prioritized_replay_beta) / args.total_timesteps
        )
            ...
            if global_step % args.train_frequency == 0:
                ...
                with torch.no_grad():
                    ...
                    next_q_values = torch.sum(
                        next_dist * support, dim=2)  # [B, num_actions]

                    # double q-learning
                    # [B, num_actions, n_atoms]
                    next_dist_online = q_network(data.next_observations)
                    next_q_online = torch.sum(
                        next_dist_online * support, dim=2)  # [B, num_actions]
                    best_actions = torch.argmax(next_q_online, dim=1)  # [B]
                    next_pmfs = next_dist[torch.arange(
                        args.batch_size), best_actions]  # [B, n_atoms]

                    # compute the n-step Bellman update.
                    gamma_n = args.gamma**args.n_step
                    next_atoms = data.rewards + gamma_n * \
                        support * (1 - data.dones.float())
                ...
                pred_dist = dist.gather(
                    1, data.actions.unsqueeze(-1).expand(-1, -1, args.n_atoms)).squeeze(1)
                ...
                if global_step % 100 == 0:
                    ...
                    q_values = (
                        pred_dist * q_network.support).sum(dim=1)  # [B]
